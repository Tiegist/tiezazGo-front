...
<template>
  <div class="admin-dashboard min-h-screen bg-[#F5F3EF] flex">
    ...
    <aside :class="[
      ...
      <div class="px-3 h-[52px] flex items-center justify-between border-b border-white/5 shrink-0">
        <div v-if="isSidebarOpen" class="flex items-center gap-2">
          <div
            class="w-7 h-7 bg-gradient-to-br from-[#E8B86D] to-[#B8860B] rounded-lg flex items-center justify-center shadow-md shadow-[#D9A05B]/20">
            <span class="text-black font-bold text-[10px]">T</span>
          </div>
          <span class="text-sm font-bold tracking-tight text-white">𝕿ዕዛዝ𝕲𝖔</span>
        </div>
        <div v-else class="w-full flex justify-center">
          <div
            class="w-7 h-7 bg-gradient-to-br from-[#E8B86D] to-[#B8860B] rounded-lg flex items-center justify-center">
            <span class="text-black font-bold text-[10px]">t</span>
          </div>
        </div>
        <button @click="isSidebarOpen = !isSidebarOpen"
          class="text-white/25 hover:text-[#D9A05B] transition-colors hidden lg:block">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24"
            stroke="currentColor" stroke-width="2">
            <path v-if="isSidebarOpen" stroke-linecap="round" stroke-linejoin="round"
      ...
      <nav class="flex-1 py-2.5 px-2 space-y-0.5 overflow-y-auto">
        <a v-for="item in navItems" :key="item.key" href="#" @click.prevent="activeNav = item.key" :class="[
          ...
          <svg v-if="item.icon === 'dashboard'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0"
          ...
          <svg v-if="item.icon === 'menu'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0"
            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path d="M3 6h18M3 12h18M3 18h18" />
          </svg>
          <svg v-if="item.icon === 'orders'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0"
            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path
              d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
          </svg>
          <svg v-if="item.icon === 'customers'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0"
            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" />
            <circle cx="9" cy="7" r="4" />
            <path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" />
          </svg>
          <svg v-if="item.icon === 'analytics'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0"
            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path d="M18 20V10M12 20V4M6 20v-6" />
          </svg>
          <svg v-if="item.icon === 'settings'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0"
            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
      ...
      <div class="px-2 py-2.5 border-t border-white/5 shrink-0">
        <button @click="logout"
          class="flex items-center gap-2.5 px-2.5 py-2 w-full rounded-lg text-[13px] font-medium text-red-400/60 hover:text-red-400 hover:bg-red-400/5 transition-all">
    ...
    <!-- Main Content -->
    <main
      :class="['flex-1 flex flex-col min-h-screen transition-all duration-300', isSidebarOpen ? 'ml-64' : 'ml-[72px]']">
      <!-- Top Header -->
      <header
        class="h-16 bg-white border-b border-gray-100 px-6 flex items-center justify-between sticky top-0 z-20 shrink-0">
        <div class="flex items-center gap-3">
          <button @click="isSidebarOpen = !isSidebarOpen"
            class="lg:hidden text-gray-400 hover:text-[#B8860B] transition-colors">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="hidden md:flex items-center bg-gray-50 border border-gray-100 rounded-lg px-3 py-1.5 gap-2 w-44">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-gray-300" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2">
              <circle cx="11" cy="11" r="8" />
              <line x1="21" y1="21" x2="16.65" y2="16.65" />
            </svg>
            <input type="text" placeholder="Search..."
              class="bg-transparent border-none outline-none text-xs text-gray-600 placeholder-gray-300 w-full" />
          </div>
          ...
          <div class="flex items-center gap-2.5 pl-3 border-l border-gray-100">
            ...
            <div
              class="w-8 h-8 rounded-lg bg-gradient-to-br from-[#E8B86D] to-[#B8860B] flex items-center justify-center text-white text-[11px] font-bold shadow-sm">
              A
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 mb-5">
          <div v-for="stat in stats" :key="stat.name"
            ...
            <div class="flex items-center justify-between mb-3">
              <div
                class="w-8 h-8 rounded-lg bg-[#D9A05B]/8 flex items-center justify-center group-hover:bg-[#D9A05B]/12 transition-colors">
                <svg v-if="stat.icon === 'revenue'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-[#B8860B]"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                  <line x1="12" y1="1" x2="12" y2="23" />
                  <path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6" />
                </svg>
                <svg v-if="stat.icon === 'orders'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-[#B8860B]"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                  <path
                    d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
                <svg v-if="stat.icon === 'customers'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-[#B8860B]"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                  <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" />
                  <circle cx="12" cy="7" r="4" />
                </svg>
                <svg v-if="stat.icon === 'tables'" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-[#B8860B]"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
          ...
          <div class="xl:col-span-2 bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-50 flex items-center justify-between">
              ...
              <div class="flex gap-1">
                <button
                  class="px-2.5 py-1 rounded-md text-xs font-medium bg-[#D9A05B]/10 text-[#B8860B] border border-[#D9A05B]/15">Week</button>
                <button
                  class="px-2.5 py-1 rounded-md text-xs font-medium text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors">Month</button>
                <button
                  class="px-2.5 py-1 rounded-md text-xs font-medium text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors">Year</button>
              </div>
            </div>
            <div class="h-[260px] flex items-end justify-around px-4 py-4 gap-2">
              <div v-for="(height, i) in [45, 65, 35, 80, 55, 90, 70]" :key="i"
                ...
                <span class="text-[11px] text-gray-300 font-medium">{{ ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][i]
                  }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
              ...
              <div v-for="(item, i) in [
                ...
                <div class="flex-1">
                  ...
                  <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      class="h-full bg-gradient-to-r from-[#D9A05B] to-[#E8B86D] rounded-full transition-all duration-500"
          ...
          <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div class="p-4 grid grid-cols-2 gap-2">
              <button
                class="flex flex-col items-center gap-2 p-3 rounded-lg bg-gray-50/50 border border-gray-100 hover:border-[#D9A05B]/30 hover:bg-[#D9A05B]/5 transition-all group">
                <svg xmlns="http://www.w3.org/2000/svg"
                  class="w-5 h-5 text-gray-300 group-hover:text-[#B8860B] transition-colors" viewBox="0 0 24 24"
